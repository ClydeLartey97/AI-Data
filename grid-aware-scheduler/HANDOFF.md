...
- The project-environment test suite is currently **126 passing**. It includes generated JavaScript parsing, persistent theme controls, the AI-operations-first route hierarchy, five-surface navigation, full-screen chart workbenches and controls, a non-blocking Fleet Lab site-signal path, modality-neutral evidence aggregation, exact capacity-aware portfolio scheduling, AI execution metadata and per-work-unit efficiency, the portfolio API/page, CAISO and NYISO propagation, hardware detection privacy/provenance, exact-fingerprint calibration, hard policy caps, canonical estimation, routing quality gates, durable decision auditing, backtest information boundaries, local HTTP routes/security headers, contiguous-window enforcement and mixed-currency rejection.
- **Current local runtime:** product version 0.8.0 is managed by the Desktop launcher at `http://localhost:8766`, PID 3277 (the launcher selects a free port on restart). AI Operations, Sites & Grid, Fleet Lab, Placement Lab, Decisions and health return HTTP 200. A live London request returned an exact AI assignment with model/version, precision, compute unit, memory fit, quality and energy/carbon/cost per useful-work unit. This is loopback-only, not a deployment.
...
- Local measurement environment: Xcode and `xctrace` are available. MLX, MLX-LM and Core ML tooling are pinned but not yet installed. The iPhone is identified; exact iPad model/OS remains to be recorded.
...
- Optional profiling dependencies are pinned in `requirements-apple.txt`: MLX 0.32.0, MLX-LM 0.31.3, Core ML Tools 9.0 and psutil 7.2.2. A dry run resolved successfully on the M2/Python 3.12 environment; packages and model weights have not yet been installed.
...
12. [ ] **CURRENT PRODUCT WORK:** the joint optimisation contract and exact pilot portfolio core are built. Next expose the workload queue, facility capacity, total cost/carbon policies and per-unit output through the versioned API and operator interface; add portfolio-level run-now and realised baselines.
...
14. [ ] Build and pin the Apple-device measurement path: a reproducible Mac runner first, followed by a minimal native iPhone/iPad collector where device support permits. Do not store prompts, media or model inputs in shared telemetry.
15. [ ] Feed measured profiles into the scheduler so it can choose model, precision, hardware, region and start time while meeting quality/deadline constraints; expose predicted versus realised energy, cost and emissions in the operator dashboard.
