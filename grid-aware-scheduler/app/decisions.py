...
.metric b{display:block;font-size:24px;letter-spacing:-.035em}.metric span,.eyebrow{color:var(--muted);font-size:11px;
letter-spacing:.055em;text-transform:uppercase}.toolbar{display:flex;gap:10px;padding:13px;margin-bottom:12px}
...
.section{margin-top:20px}.section h4{font-size:12px;text-transform:uppercase;letter-spacing:.055em;color:var(--muted);margin:0 0 10px}
...
<div class="toolbar card"><input id="search" type="search" placeholder="Search ID, model, hardware or location" aria-label="Search decisions"><select id="status" aria-label="Filter by status"><option value="all">All evidence states</option><option value="scored">Scored</option><option value="awaiting_outturn">Awaiting outturn</option></select><button id="refresh" type="button">Refresh</button></div>
<div class="split"><section class="card table-wrap"><table><thead><tr><th>Created</th><th>Market</th><th>Workload</th><th>Placement</th><th>Scheduled</th><th>Forecast cost</th><th>Forecast carbon</th><th>Evidence</th></tr></thead><tbody id="rows"></tbody></table><div class="empty" id="empty">Loading decisions…</div></section>
...
document.getElementById("search").addEventListener("input",renderRows);document.getElementById("status").addEventListener("change",renderRows);document.getElementById("refresh").addEventListener("click",load);load()})();
</script></body></html>"""
